...
export default function ViewPricingWithDraw({
  item,
  setData,
  dataOld,
  setDataOld,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(response.data.withdraw.user.withdraws[0].status);
      console.log(dataOld);
      const update = dataOld.map((value) => {
        if (value.id === item.id) {
          value.status = response.data.withdraw.user.withdraws[0].status;
        }
        return value;
      });
      setDataOld(update);
