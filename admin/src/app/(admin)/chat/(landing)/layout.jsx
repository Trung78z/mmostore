...
const schema = z.object({
  name: z
    ...
    .max(15, { message: "Vui lòng nhập nhiều nhất 15 kí tự!" }),
});
    ...
    <div>
      {authState.status === true ? (
        <div className="mx-auto my-4 w-full max-w-screen-lg pt-4">
          <div className="grid h-full min-h-[80vh] grid-cols-1 md:grid-cols-7">
            <div className="chat-container relative hidden max-h-[80vh] space-y-2 overflow-y-auto overflow-x-hidden border-r-4 bg-background md:col-span-2 md:block">
              {rooms.length > 0 ? (
                rooms.map((item) => {
                  return (
                    <div key={item.id} className="relative">
                      <Link
                        ...
                        className={cn(
                          ...
                          parseInt(slug) === item.roomId && "bg-slate-200",
                        )}
                      >
                        ...
                        <h2 className="text-lg font-normal">
                          {/* {item.room.members[1]
                            ? `${item.room?.members[1].user.profiles.firstName} ${item.room?.members[1].user.profiles.lastName}`
                            : item.room.name} */}
                          {item.room.name}
                          {/* {getRoomName(item.room, authState.id)} */}
                        </h2>
                      </Link>
                      <Button
                              ...
                              {listUserRoom.map((item) => {
                                return (
                                  ...
                                  >
                                    ...
                                    <Button
                                      className="rounded-full p-1"
                                      onClick={() => handleSubUserRoom(item.id)}
                                    >
                                      <RiSubtractFill className="h-4 w-4 rounded-full bg-red-500" />
                                    </Button>
                                  </li>
        ...
        <div className="flex min-h-[70vh] items-center justify-center">
          <h3>{"Vui lòng đăng nhập để chat với tư vấn viên"}</h3>
          <Button
