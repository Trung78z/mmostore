...
export default function DonHangSanPham() {
  ...
  const fetch = async () => {
    try {
      const response = await axios.get("/orders/service", {
    ...
    <div className="space-y-2 px-2 py-4">
      <div className="space-y-2">
        <div className="card flex flex-col items-center justify-between md:flex-row">
          <h1 className="flex-1 text-2xl font-medium">Dịch vụ</h1>
            ...
            <TableBody>
              {data.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell className="text-center">
                    {(item.total * (100 - parseInt(item.services?.receiving))) /
                      100}
                  </TableCell>
                  <TableCell className="text-center">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleActive(
                          item.id,
                          item.total,
                          item.services.userId,
                          item.services.receiving,
                        )
                      }
                    >
                      <FaCheckCircle />
