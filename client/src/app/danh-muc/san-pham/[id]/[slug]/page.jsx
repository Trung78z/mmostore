      ...
      if (res.value) {
        try {
          ...
          Swal.fire({
            title: "Success!",
            html: `
              <strong>Chúng tôi vừa gửi cho bạn thông tin tài khoản bạn đã mua trong tệp Excel!</strong>
              <p>Đặt hàng thành công!</p>
              <p>Đơn hàng của bạn là: <strong>${response.data.msg.id}</strong></p>
              <p>Tổng số tiền bạn đã đặt hàng total:<strong> ${price * parseInt(amount) - 0} </strong></p> 
            `,
            icon: "success",
          });
          const fileResponse = await axios.get(response.data.msg.fileUrl, {
            headers: {
              Authorization: "Bearer " + sessionStorage.getItem("token"),
            },
            responseType: "blob", // Đổi từ "stream" sang "blob" cho dữ liệu tải về
          });

          // Tạo URL và liên kết để tải file
          const url = window.URL.createObjectURL(new Blob([fileResponse.data]));
          const link = document.createElement("a");
          link.href = url;
          link.setAttribute("download", "account.csv"); // Đổi tên file theo ý muốn
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
          setAuthState({
            ...authState,
            accountBalance: response.data.msg.updatedProfile.accountBalance,
          });
          setBuy(true);
        } catch (error) {
